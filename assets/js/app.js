...
(function(){
  ...
  function initChess(){
    ...
    let cpuOn=false;
    const cpuToggle=document.getElementById('chessCpu'); if(cpuToggle){ cpuOn=cpuToggle.checked; cpuToggle.addEventListener('change',()=>cpuOn=cpuToggle.checked); }
    board.addEventListener('click',e=>{
      ...
      if(sel){ // second click
        ...
        if(MicroChess.move(r0,c0,r,c)){ render();
          // CPU replies
          if(cpuOn){ setTimeout(()=>{ MicroChess.cpuMove(); render(); }, 300); }
        }
  ...
  function initTheme(){
    const themes=['theme-indigo','theme-teal','theme-amber','theme-deeppurple'];
    const btn=document.getElementById('themeBtn');
    const saved=localStorage.getItem('theme')||themes[0];
    document.documentElement.classList.add(saved);
    btn.addEventListener('click',()=>{
      const cur=themes.indexOf([...document.documentElement.classList].find(c=>themes.includes(c))||themes[0]);
      const next=themes[(cur+1)%themes.length];
      themes.forEach(t=>document.documentElement.classList.remove(t));
      document.documentElement.classList.add(next);
      localStorage.setItem('theme',next);
      new Audio('assets/sounds/tap.wav').play().catch(()=>{});
    });
  }
